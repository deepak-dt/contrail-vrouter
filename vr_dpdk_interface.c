...
uint16_t dpdk_if_tx_callback(uint8_t port_id, uint16_t queue, struct rte_mbuf *pkts[], uint16_t nb_pkts, void *user_param)
{
...
uint16_t dpdk_if_rx_callback(uint8_t port_id, uint16_t queue, struct rte_mbuf *pkts[], uint16_t nb_pkts, uint16_t max_pkts, void *user_param)
{
    const unsigned lcore_id = rte_lcore_id();
    struct vr_dpdk_lcore * const lcore = vr_dpdk.lcores[lcore_id];
    struct vr_interface *bond_vif = (struct vr_interface *)user_param;
...
dpdk_monitoring_if_add(struct vr_interface *vif)
{
    ...
    if (ret == 0) {
        /* Check if the monitored interface is a slave interface of some bond interface.
           If yes then register Rx Tx call-back functions for the specified slave interface per Rx Tx queues eac*/
        if (NULL != (slave_substr = strstr((const char*)vif->vif_name, "slave"))) {
            slave_port_id = strtol((slave_substr+5), &slave_port_str, 10);
        }
        ...
        if (bonded_ethdev->ethdev_nb_slaves > 0) {
            for (q_id = 0; q_id < bonded_ethdev->ethdev_nb_rx_queues; q_id++) {
                if (!(vr_dpdk.slave_monitorings[slave_port_id].rx_cb[q_id] = rte_eth_add_rx_callback(slave_port_id, q_id, &dpdk_if_rx_callback, (void*)monitored_vif))) {
        ...
        struct vr_interface_settings *settings)
{
    ...
    memset(settings->bond_slaves, 0, sizeof(settings->bond_slaves));

    settings->nbr_bond_slaves = rte_eth_bond_slaves_get(vif->vif_idx, bond_slaves, sizeof(bond_slaves));

    for (slave_index = 0; slave_index < settings->nbr_bond_slaves; slave_index++) {
        settings->bond_slaves[slave_index].port_id = bond_slaves[slave_index];
        rte_eth_macaddr_get(bond_slaves[slave_index], &mac_addr);
        memcpy(settings->bond_slaves[slave_index].mac_addr, mac_addr.addr_bytes, ETHER_ADDR_LEN);
    }
