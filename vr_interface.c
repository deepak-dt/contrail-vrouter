...
__vr_interface_make_req(vr_interface_req *req, struct vr_interface *intf,
        unsigned int core, struct vr_bond_slave_settings *bond_slave_info)
{
    uint8_t proto;
    uint16_t port;
    ...
    if (req->vifr_name) {
        if ( NULL != bond_slave_info) {
            if (req->vifr_type == VIF_TYPE_PHYSICAL) {
                snprintf(req->vifr_name, VR_INTERFACE_NAME_LEN - 1, "vif%d/%d/slave%d", req->vifr_rid, req->vifr_idx, bond_slave_info->port_id);
            } else {
                strncpy(req->vifr_name, intf->vif_name, VR_INTERFACE_NAME_LEN - 1);
            }

            memcpy(req->vifr_mac, bond_slave_info->mac_addr, VR_ETHER_ALEN);
        } else {
            strncpy(req->vifr_name, intf->vif_name, VR_INTERFACE_NAME_LEN - 1);
        }
    }
        ...
        unsigned int core, struct vr_bond_slave_settings *bond_slave_info)
{
    ...
    __vr_interface_make_req(req, vif, core, bond_slave_info);
...
vr_interface_get(vr_interface_req *req)
{
    ...
    uint32_t slave_port_id = 0;
    int slave_index = 0;
    struct vr_bond_slave_settings *bond_slave_info = NULL;
    ...
    //Check if interface to get is a slave interface. If yes then pass on its eth_port_id to function.
    if (req->vifr_name) {
        if (NULL != (slave_substr = strstr(req->vifr_name, "slave"))) {
            sscanf(slave_substr, "%u", &slave_port_id);
            ...
            if (!hif_ops->hif_get_settings(vif_os, &settings)) {
                vr_printf("Error in fetching settings for vif: %u, vif_os: %u\n", vif->vif_idx, vif_os->vif_idx);
            }

            for (slave_index = 0; slave_index < settings.nbr_bond_slaves; slave_index++) {
                if (settings.bond_slaves[slave_index].port_id == slave_port_id) {
                    bond_slave_info = &(settings.bond_slaves[slave_index]);
                    break;
                }
            }
        }
    ...
    ret = vr_interface_make_req(vif_resp, vif, (unsigned)(req->vifr_core - 1), bond_slave_info);
...
vr_interface_dump(vr_interface_req *r)
{
    ...
    for (i = (unsigned int)(r->vifr_marker + 1);
        ...
        if (vif) {
            ...
            vr_interface_make_req(resp, vif, (unsigned)(r->vifr_core - 1), bond_slave_info);
            ...
            if (vif->vif_type == VIF_TYPE_PHYSICAL) {
                for (slave_index = 0; slave_index < settings.nbr_bond_slaves; slave_index++) {
                    /* zero vifr_core means to sum up all the per-core stats */
                    vr_interface_make_req(resp, vif, (unsigned)(r->vifr_core - 1), &(settings.bond_slaves[slave_index]));
