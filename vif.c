...
list_get_print(vr_interface_req *req)
{
    ...
    unsigned char slave_port_id = 0;
    unsigned int vifr_os_idx;

    vifr_os_idx = req->vifr_os_idx;

    if (req->vifr_name) {
        if (NULL != (slave_substr = strstr(req->vifr_name, "slave"))) {
            printed += printf("/%s", slave_substr);
            vifr_os_idx = strtol(slave_substr, &slave_port_str, 10);
        }
    }
    if (rate_set) {
        print_zero = true;
        ...
        switch (req->vifr_type) {
            case VIF_TYPE_PHYSICAL:
                ...
                printf("PCI: ""%.4" PRIx16 ":%.2" PRIx8 ":%.2" PRIx8 ".%" PRIx8,
                        (uint16_t)(vifr_os_idx >> 16),
                        (uint8_t)(vifr_os_idx >> 8) & 0xFF,
                        (uint8_t)(vifr_os_idx >> 3) & 0x1F,
                        (uint8_t)(vifr_os_idx & 0x7));
                break;
